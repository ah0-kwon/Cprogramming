...
#include<stdio.h>
```
-표준 입출력 함수(printf 등)를 사용하기 위한 헤더 파일 포함
```
int main(void)
```
-메인함수 시작
...
char a = 'A';
```
-char형(1바이트 문자기본형) 변수 a를 선언하고 문자 'A'로 초기화
```
int b = 36;
```
-int형(4바이트 정수형) 변수 b를 선언하고 정수 36으로 초기화
```
double c = 3.141592;
```
-double형(8바이트 실수형) 변수 c를 선언하고 실수 3.141592로 초기화
```
printf("char형 변수 a의 주소 : %u\n", &a);
```
-&a를 통해 변수 a의 메모리 주소를 구하고, %u(부호 없는 정수) 서식으로 출력
```
printf("int형 변수 b의 주소: %u\n", &b);
```
-&b를 통해 변수 b의 메모리 주소를 구하고, %u 서식으로 출력
```
printf("double형 변수 c의 주소: %u\n", &c);
```
-&c를 통해 변수 c의 메모리 주소를 구하고, %u 서식으로 출력
```
return 0;
```
-프로그램 종료(0반환)
